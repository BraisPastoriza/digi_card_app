...
void main() {
  group('buildNumberSort', () {
    ...
    List<String> sorted(List<String> numbers) =>
        numbers.toList()
          ..sort(
            (a, b) => buildNumberSort(a, 0).compareTo(buildNumberSort(b, 0)),
          );

    test('orders card numbers numerically, not lexicographically', () {
      expect(
        sorted(['BT1-010', 'BT1-002', 'BT1-100', 'BT1-009']),
        ['BT1-002', 'BT1-009', 'BT1-010', 'BT1-100'],
      );
    });

    test('orders set numbers numerically', () {
      expect(
        sorted(['BT10-001', 'BT9-001', 'BT2-001']),
        ['BT2-001', 'BT9-001', 'BT10-001'],
      );
    });

    test('groups by set prefix before number', () {
      expect(
        sorted(['ST1-01', 'BT1-001', 'EX1-001', 'AD1-001']),
        ['AD1-001', 'BT1-001', 'EX1-001', 'ST1-01'],
      );
    });
