...
final deckProvider = StreamProvider.family<Deck?, int>(
  (ref, deckId) => ref.watch(deckDaoProvider).watchDeck(deckId),
);

/// The cards in one revision, resolved against the library.
final revisionEntriesProvider =
    StreamProvider.family<List<DeckEntry>, int>(
      (ref, revisionId) =>
...
final compositionProvider = Provider.family<AsyncValue<DeckComposition>, int>(
  (ref, revisionId) => ref
...
final revisionQuantitiesProvider =
    Provider.family<Map<String, int>, int>((ref, revisionId) {
      final entries = ref
          .watch(revisionEntriesProvider(revisionId))
          .valueOrNull;
